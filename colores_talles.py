...
nombre_archivo = input("Introduce el nombre del archivo Excel (SIN la extensión .xlsx): ")
archivo_entrada = f"{nombre_archivo}.xlsx"
archivo_salida = f"{nombre_archivo}_actualizado.xlsx"
...
# Ordenamos por longitud para dar prioridad a colores compuestos ("AZUL OSCURO" antes que "AZUL")
...
try:
    ...
    if columna_color in df.columns:
        ...
        print(f"✔️ Columna '{columna_codigo_color}' creada a la izquierda de '{columna_color}'.")
    else:
        print(f"❌ Advertencia: No se encontró la columna '{columna_color}' en el Excel.")

    # --- 2. PROCESAR LA COLUMNA TALLE (Preservando la original) ---
    if columna_talle in df.columns:
        ...
        print(f"✔️ Columna '{columna_codigo_talle}' creada a la izquierda de '{columna_talle}'.")
    else:
    ...
    if columna_barras in df.columns:
        ...
        print(f"✔️ Columna '{columna_barras}' procesada (se agregaron comillas simples al inicio de los números).")
    ...
    if columna_comag in df.columns and columna_codigo_talle in df.columns and columna_codigo_color in df.columns:
        ...
        # OBTENEMOS LAS POSICIONES DESPUÉS DE HABER INSERTADO LAS COLUMNAS NUEVAS
        idx_comag = df.columns.get_loc(columna_comag)
        idx_talle_nuevo = df.columns.get_loc(columna_codigo_talle) # ¡Ahora apunta a codigo_talle!
        ...
        print(f"✔️ Columna 'Codigo_Completo' creada como fórmula dinámica en la Posición A.")

    # Guardar el nuevo archivo
    df.to_excel(archivo_salida, index=False)
    print(f"\n¡Éxito! El archivo modificado se guardó como: {archivo_salida}")

except FileNotFoundError:
    print(f"\nError: No se encontró el archivo '{archivo_entrada}'. Asegúrate de que esté en la misma carpeta que este script.")
